...
void Display()
{
	...
	if ( g_camera_mode == 1 )	//1인칭
	{
		// Projection Transform Matrix 설정.
		projection_matrix = glm::perspective(glm::radians(g_cameraRadian), (float)g_window_w / g_window_h, 0.01f, 10000.f);
		glUniformMatrix4fv(m_proj_loc, 1, GL_FALSE, glm::value_ptr(projection_matrix));

		// Camera Transform Matrix 설정.
		glm::vec3 r1 = glm::rotateY(g_ro, g_car_rotation_y);
		glm::vec3 camera2pos = r1 + g_car_poisition;
		glm::vec3 r2 = glm::rotateY(g_cameralook, g_car_rotation_y);
		glm::vec3 camera2look = r2 + g_car_poisition;

		view_matrix = glm::lookAt(camera2pos, camera2look, glm::vec3(0.f, 1.f, 0.f));
		glUniformMatrix4fv(m_view_loc, 1, GL_FALSE, glm::value_ptr(view_matrix));
	}

	else if (g_camera_mode == 2)	//3인칭
	{
		// Projection Transform Matrix 설정.
		projection_matrix = glm::perspective(glm::radians(g_cameraRadian), (float)g_window_w / g_window_h, 0.01f, 10000.f);
		glUniformMatrix4fv(m_proj_loc, 1, GL_FALSE, glm::value_ptr(projection_matrix));
...
void Timer(int value)
{
	...
	// Calculate VelocityGetGLViewMatrix
	...
	glm::vec3 camera_v;
	float radian_v;
	glm::vec3 cameralook_v;

	if (g_camera_mode == 1) {
		camera_v = (glm::vec3(0, 0.6f, -0.16f) - g_ro) / 20.f;
		g_ro += camera_v;
		radian_v = (90.f - g_cameraRadian) / 20.f;
		g_cameraRadian += radian_v;

		cameralook_v = (glm::vec3(0, 1.5f, 6.0f) - g_cameralook) / 20.f;
		g_cameralook += cameralook_v;
	}
	else if (g_camera_mode == 2) {
		camera_v = (glm::vec3(0, 2.0f, -3.0f) - g_ro) / 20.f;
		g_ro += camera_v;
		radian_v = (45.f - g_cameraRadian) / 20.f;
		g_cameraRadian += radian_v;
		cameralook_v = (glm::vec3(0, 0, 3.0f) - g_cameralook) / 20.f;
		g_cameralook += cameralook_v;
	}
